      ...
      <header className="h-16 border-b-2 border-white/10 flex items-center justify-between px-4 shrink-0 bg-black z-50">
        ...
        <div className="flex-1 max-w-xl mx-8 relative hidden md:block">
          <input 
            ...
            className="w-full bg-white/5 border border-white/20 px-4 py-2 pl-10 rounded-none focus:outline-none focus:border-cyan-400 focus:bg-white/10 transition-all font-sans"
          />
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`px-4 py-2 border-2 text-sm font-bold transition-all ${isShortsOpen ? 'bg-cyan-400 text-black border-cyan-400' : 'bg-transparent text-white border-white/20 hover:border-white'}`}
          >
            SHORTS
          </button>
        </div>
          ...
          <aside className="w-64 border-r-2 border-white/10 flex flex-col bg-black/95 z-40 overflow-y-auto shrink-0">
              ...
              {session?.user ? (
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="overflow-hidden">
                      <p className="font-bold truncate text-cyan-400">{session.user.name}</p>
                  ...
                  <button className="w-full flex items-center justify-center gap-2 p-3 bg-white text-black font-bold hover:bg-cyan-400 transition-colors mt-2">
                ...
                <div className="flex flex-col gap-3">
                  <button 
                    ...
                    className="w-full flex items-center gap-3 p-3 border-2 border-cyan-400 hover:bg-cyan-400 hover:text-black transition-colors group"
                  >
                  ...
                  <div className="p-3 border border-white/10 bg-white/5 text-xs text-center text-white/70">
                    <span className="text-cyan-400 font-bold">PROMO:</span> Первые 30 дней в топе ленты бесплатно после авторизации!
                  </div>
                </div>
              )}
            </div>
                ...
                {CATEGORIES.map(cat => {
                  ...
                  return (
                    <button 
                      ...
                      className={`flex items-center gap-3 p-3 transition-all border-l-4 ${activeCategory === cat.id ? 'border-cyan-400 bg-white/5' : 'border-transparent hover:bg-white/5 hover:border-white/20'}`}
                    >
            ...
            <div className="mt-auto border-t-2 border-white/10 p-4 bg-gradient-to-t from-cyan-900/20 to-transparent">
              ...
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="flex items-center gap-3 p-3 border-2 border-cyan-400/50 hover:border-cyan-400 bg-black transition-all group"
              >
                <div className="w-8 h-8 bg-cyan-400 rounded-full flex items-center justify-center text-black font-black">
                  TF
                </div>
                <div>
                  <p className="font-bold text-sm group-hover:text-cyan-400 transition-colors">TaFeedRoom</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredStreams.map(stream => (
                <div key={stream.id} className="group relative bg-black border-2 border-white/10 hover:border-cyan-400 transition-colors aspect-video flex flex-col cursor-pointer">
                  ...
                  <div className="absolute inset-0 border-4 border-cyan-400 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity"></div>
                </div>
              ))}
            </div>
              ...
              {[1, 2, 3].map(i => (
                <div key={i} className="h-full w-full snap-start border-b border-white/5 relative bg-zinc-900 group flex items-center justify-center">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/90 to-transparent">
                    <p className="text-cyan-400 font-bold text-xs mb-1">@xQc</p>
                    <p className="text-sm font-bold truncate">CRAZY JUMP SCARE 😱</p>
                  </div>

                  <div className="absolute right-4 bottom-20 flex flex-col gap-4">
                    <button className="p-3 bg-black/50 backdrop-blur rounded-full hover:bg-cyan-400 hover:text-black transition-colors">
